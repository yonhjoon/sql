...
AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
   ...
   JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
   JOIN NATIONAL USING (NATIONAL_CODE);
-- 권한을 주지않아 grant you the necessary privileges 에러발생 권한 줘야함

GRANT CREATE VIEW TO KH; -- 뷰 권한 부여하는코드
...
CREATE OR REPLACE VIEW VW_EMPLOYEE -- 생성 또는 추가 뷰를 만들때 이렇게 만들면 없다면 생성되고 있다면 덮어씌이게된다.
...
/*
    * 뷰 컬럼에 별칭부여
    서브쿼리의 SELECT절에 함수식이나 산술연삭식이 기술되어있을 경우 반드시 별칭을 지정해야한다.
*/
...
-- 뷰를 통해서 조작하게되면 실제 데이터가 담겨있는 테이블에 반영이된다.
...
/*
    * DML 명령어로 조작이 불가능한 경우가 많다
    ...
    6) JOIN을 이용햇 ㅓ여러테이블을 연결시켜놓은 경우
    
    대부분 뷰는 조회를 목적을 생성한다. 그냥 뷰를 통한 DML은 안쓰는게 좋다.
*/
/*
    VIEW 옵션
    ...
    3) WITH CHECK OPION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능하도록
...
AS SELECT TCODE, TNAME, TCONTENT
    FROM TT; -- 이렇게만 한다면 테이블이 없다고 오류가 남

--서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 우선은 생기게 된다.
CREATE OR REPLACE FORCE VIEW VW_EMP
AS SELECT TCODE, TNAME, TCONTENT
    FROM TT; -- 경고: 컴파일 오류와 함께 뷰가 생성되었습니다. 문구가 뜸
    
SELECT * FROM VW_EMP; -- 실제 생성은 안됨
...
-- 3) WITH CHECK OPION : 서브쿼리에 기술된 조건에 부합하지 않은 값으로 수정시 오류 발생
-- WITH CHECK OPION 안씀
